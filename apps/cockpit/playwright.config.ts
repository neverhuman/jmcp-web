...
// traffic is mocked inside the page (see e2e/mock-broker.ts); no live :18877 is
// required.
const PORT = 15999;
...
export default defineConfig({
  ...
  webServer: {
    // Build first, then serve the static build with `vite preview`. The preview
    // server is fully offline; the broker is mocked per-page via page.route.
