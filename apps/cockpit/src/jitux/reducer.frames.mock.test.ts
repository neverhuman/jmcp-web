...
describe("JITUX reducer frames", () => {
  it("reduces every canonical frame family and ignores old frames", () => {
    ...
    const oldFrameResult = reduceJituxFrame(
      state,
      frame(13, {
        type: "session.done",
    ...
    expect(oldFrameResult.lastSeq).toBe(14);
    expect(oldFrameResult.error).toBe("The session stopped.");
  });
